...
void UAuraAbilitySystemComponent::UpgradeAttribute(const FGameplayTag& AttributeTag)
{
	...
	{
		if (IPlayerInterface::Execute_GetAttribiutePoints(GetAvatarActor()) > 0)
		{
...
void UAuraAbilitySystemComponent::ServerSpendSpellPoint_Implementation(const FGameplayTag& AbilityTag)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		ClientUpdateAbilityStatus_Implementation(AbilityTag, GetStatusTagFromSpec(*AbilitySpec), AbilitySpec->Level);
		MarkAbilitySpecDirty(*AbilitySpec);
	}
}
...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag,	const FGameplayTag& SlotTag)
{
	if (FGameplayAbilitySpec* AbilitySpec =  GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			if (!SlotIsEmpty(SlotTag)) // there is ability. Deactivate and clear its slot.
			{
				if (FGameplayAbilitySpec* SpecWithSlot = GetSpecWithSlot(SlotTag))
				{
					//if new ability is the same as the old one, do nothing
			...
			if (!AbilityHasAnySlot(AbilitySpec)) // doesn't have a slot - it's not active
			{
...
bool UAuraAbilitySystemComponent::GetDescriptionsByAbilityTag(const FGameplayTag& AbilityTag, FString& OutDescriptions,
                                                              FString& OutNextLevelescriptions)
{
	if (const FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		if (UAuraGameplayAbility* AuraAbility = Cast<UAuraGameplayAbility>(AbilitySpec->Ability))
		{
			OutDescriptions = AuraAbility->GetDescription(AbilitySpec->Level);
			OutNextLevelescriptions = AuraAbility->GetNextLevelDescription(AbilitySpec->Level + 1);
			return true;
		}
	}
	...
	{
		OutDescriptions =  UAuraGameplayAbility::GetLockerDescription(AbilityInfo->FindAbilityInfoForTag(AbilityTag).LevelRequirement);
	}	
	OutNextLevelescriptions = FString();
	return false;
}
