	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -0.5f)
		{
			...
			if (bDebuffSuccess)
			{
				...
				const float DebuffDamage = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Damage, false, 0.0f);
				const float DebuffDuration = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Duration, false, 0.0f);
				const float DebuffFrequency = Spec.GetSetByCallerMagnitude(GameplayTags.Debuff_Frequency, false, 0.0f);
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	{	
		...
		{
			// 1. Ovverride TakeDamage in AuraCharacterClass *
			// 2. Create delegate OnDamage on the Victim here *
			// 3. Bind Lambda to OnDamageDelegate on a Victim here *
			// 4. Call UGameplayStatics::ApplyRadialDamageWithFallof to cause damage(
			//(this will result on the Victim, which will then broadcast OnDamageDelegate)
