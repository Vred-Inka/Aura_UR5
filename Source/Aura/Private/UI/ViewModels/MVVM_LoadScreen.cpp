...
void UMVVM_LoadScreen::NewSlotButtonPressed(int32 Slot, const FString& EnteredName)
{
	...
	{
		GEngine->AddOnScreenDebugMessage(1, 15.0, FColor::Magenta,
			FString("Please switch to single player!"));
		return;
	}
	...
	{
		GEngine->AddOnScreenDebugMessage(1, 15.0, FColor::Magenta,
			FString("Please enter player name!"));
		return;
	}
	
	LoadSlots[Slot]->SetPlayerName(EnteredName);
	LoadSlots[Slot]->SetPlayerLevel(1);
	LoadSlots[Slot]->SetMapName(AuraGameMode->DefaultWorldName);
	LoadSlots[Slot]->SlotStatus = Taken;
	LoadSlots[Slot]->PlayerStartTag = AuraGameMode->DefaultPlayerStartTag;
	LoadSlots[Slot]->MapAssetName = AuraGameMode->DefaultWorld.ToSoftObjectPath().GetAssetName();
...
void UMVVM_LoadScreen::LoadData()
{
	...
	for (const TTuple<int32, UMVVM_LoadSlot*>& LoadSlot : LoadSlots)
	{
		if (ULoadScreenSaveGame* SaveObject = AuraGameMode->GetSaveSlotData(LoadSlot.Value->GetLoadSlotName(), LoadSlot.Key))
		{
...
{
	UE_MVVM_SET_PROPERTY_VALUE(NumLoadSlots, NumLoadSlots);
}
