...
TArray<USceneComponent*> APointCollection::GetGroundPoints(const FVector& GroundLocation, int32 NumPoints,
	float YawOverride)
{
	checkf(ImmutablePts.Num() >= NumPoints, TEXT("Attempt to access immutable points out of bounds"));
	...
	for (USceneComponent* Pt : ImmutablePts)
	{
		...
		UAuraAbilitySystemLibrary::GetLivePlayersWithinRadius(this, ActorsToIgnore,
			TArray<AActor*>(), 1500.0f, GetActorLocation() );

		FCollisionQueryParams QueryParams;
		QueryParams.AddIgnoredActors(ActorsToIgnore);
		GetWorld()->LineTraceSingleByProfile(HitResult, Raisedlocation, Loweredlocation, FName("BlockAll"), QueryParams);

		const FVector AdjustedLocation =FVector(Pt->GetComponentLocation().X, HitResult.ImpactPoint.Y, HitResult.ImpactPoint.Z);
